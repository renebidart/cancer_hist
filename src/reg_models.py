    ...
    while 1:
        ...
        for batch in range(num_batches):
            ...
            for image_file in batch_files:
                x_temp, y_temp = get_imgs_noloc(image_file, num_regions = batch_size/num_img, im_size = image_shape[0])
                y.extend(y_temp)
                x.extend(x_temp)
            x=np.array(x)
            y=np.array(y)
...
def data_gen_loc_no_aug(file_loc, batch_size, image_shape=(100, 100)):
    ...
    while 1:
        ...
        for batch in range(num_batches):
            ...
            for image_file in batch_files:
                x_temp, y_temp = get_imgs_noloc_aug(image_file, num_regions = batch_size/num_img, im_size = image_shape[0])
                y.extend(y_temp)
                x.extend(x_temp)
            x=np.array(x)
            y=np.array(y)
